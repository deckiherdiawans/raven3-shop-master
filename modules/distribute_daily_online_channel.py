...
def distribute_daily_online_channel(date, top_limit):
    # AMBIL DATA DARI STORED PROCEDURE DAN HITUNG JUMLAH DATANYA
    result = executeQuery("raven_getOnlineByChannel '%s'" % date)
    total_data = len(result)

    # PERSIAPKAN ARRAY UNTUK MENAMPUNG DATA
    data = {
        'title' : 'Daily Online Channel'
    }

    # JIKA DATA TIDAK NOL
    if total_data > 0:

        data['table_headers'] = ['#', 'Channel', 'Qty', 'Sales Value']
        data['records'] = []
        ...
        for item in result:
            ...
            if counter <= top_limit:
                ...
                row = [counter, channel, qty, sales_value]
        ...
        if (total_data > top_limit):
            ...
            row = [counter, channel, qty, others_sales_value]
            data['records'].append(row)
